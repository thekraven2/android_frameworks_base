...
class OpenvpnService extends VpnService<OpenvpnProfile> {
    ...
    void startConnectivityMonitor() {
        /*
         * Openvpn is completely event driven, so we don't need a polling
    ...
    private class OpenvpnThread extends Thread {
        ...
        private synchronized void signalState(String s) {
            ...
            /*
             * state can be: CONNECTING -- OpenVPN's initial state. WAIT --
             * (Client only) Waiting for initial response from server. AUTH --
             * (Client only) Authenticating with server. GET_CONFIG -- (Client
             * only) Downloading configuration options from server. ASSIGN_IP --
             * Assigning IP address to virtual network interface. ADD_ROUTES --
             * Adding routes to system. CONNECTED -- Initialization Sequence
             * Completed. RECONNECTING -- A restart has occurred. EXITING -- A
             * graceful exit is in progress. Really all we care about is
             * connected or not
             */
            vpnState = state;
            this.notifyAll();
        ...
        private synchronized void signalPassword(String s) throws IOException {
            /*
             * message should be Need '<auth type>' password but coult be
             * Verification Failed: '<auth type'
             */
            ...
            /*
             * AuthType can be one of "Auth" - regular client server
             * authentication "Private Key" - password for private key
             * (unimplemented)
             */

            if (s.startsWith("Need")) {
                /*
                 * we're in the processor thread, so we have to send these
                 * asynchronously to avoid a deadlock
                 */
